...
##############################################################################
# Build PLUMED from source
#
# PLUMED is built with the host MPI C++ compiler (mpicxx), NOT hipcc, since it
# is plain CPU/C++ code. We install it to its own prefix and let the LAMMPS
# CMake build locate it via pkg-config (PKG_CONFIG_PATH below).
#
# We use PLUMED_MODE=runtime in the LAMMPS build: LAMMPS only embeds the small
# PLUMED wrapper at compile time and dlopen()s the kernel at runtime. This
# avoids C++ ABI mismatches between hipcc (clang-based) and gcc/mpicxx, and
# only requires the PLUMED kernel to be findable at runtime.
##############################################################################
...
# Make this PLUMED installation discoverable by LAMMPS' CMake (pkg-config) and
# at runtime.
